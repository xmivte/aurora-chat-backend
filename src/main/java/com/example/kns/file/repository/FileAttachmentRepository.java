...
public interface FileAttachmentRepository {
	...
	@Select("""
			...
			ORDER BY message_id, uploaded_at ASC
			</script>
			""")
	List<FileAttachment> findByMessageIds(@Param("messageIds") List<Long> messageIds);
	...
	@Select("""
			SELECT id, message_id AS messageId, file_name AS fileName,
			       ...
			       uploaded_at AS uploadedAt, expires_at AS expiresAt
			FROM db.file_attachments
			WHERE uploaded_at < #{expirationDate}
			""")
	List<FileAttachment> findByUploadedAtBefore(@Param("expirationDate") LocalDateTime expirationDate);
